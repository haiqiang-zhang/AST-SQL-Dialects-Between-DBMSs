...
-- For BKA, verify that the multi-equality rewriting of the MRR iterator
-- (which is really unneeded in this case, but still done) doesn't change
-- the condition on t1.
let $query =
...
-- Verify that we've got BKA (in join_outer_bka.test) on the innermost join,
-- but not on the outermost, because that join is not right-deep.
--skip_if_hypergraph  -- No BKA support.
eval EXPLAIN format=tree $query;

DROP TABLE t1, t2;

-- Same test as in hash_join.test, just for nested loop join.
