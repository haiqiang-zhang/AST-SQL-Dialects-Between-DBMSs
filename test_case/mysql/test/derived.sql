...
SELECT MIN(price) min, MAX(price) max, AVG(price) avg FROM (SELECT SUBSTRING( MAX(concat(date_,";
DROP TABLE t1;
...
-- Test EXPLAIN on MaterializeIterator.
...
-- A merge hint overrides the heuristic to not merge.
...
-- ot.t2_id is a deeply nested outer reference.

-- We verify that the pulling-out finds the outer reference. We hide
-- this reference in various places :-)

-- Inside the WHERE clause of a merged derived table in a scalar subquery.
...
-- Inside the WHERE clause of a non-merged derived table in a scalar
-- subquery.
