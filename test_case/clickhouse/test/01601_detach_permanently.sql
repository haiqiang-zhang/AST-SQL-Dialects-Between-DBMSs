...
drop table test1601_detach_permanently_atomic.test_name_reuse;
...
ATTACH TABLE test1601_detach_permanently_atomic.test_name_reuse UUID '00000000-0000-0000-0000-000000000001'ã(`number` UInt64ã)ãENGINE = MergeTreeãORDER BY tuple()ãSETTINGS index_granularity = 8192;
...
ATTACH TABLE test1601_detach_permanently_ordinary.test_name_reuse (`number` UInt64ã)ãENGINE = MergeTreeãORDER BY tuple()ãSETTINGS index_granularity = 8192;
DETACH table test1601_detach_permanently_ordinary.test_name_reuse PERMANENTLY;
SELECT 'can attach with short syntax';
...
SELECT 'can still show the create statement';
SELECT 'can attach with full syntax';
ATTACH TABLE test1601_detach_permanently_lazy.test_name_reuse (`number` UInt64ã)ãENGINE = Log;
DETACH table test1601_detach_permanently_lazy.test_name_reuse PERMANENTLY;
SELECT 'can attach with short syntax';
