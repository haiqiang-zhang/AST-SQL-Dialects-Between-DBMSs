...
select * from (SELECT number as a FROM numbers(10)) t1 ALL PASTE JOIN (select number as a from numbers(10)) t2;
TRUNCATE TABLE test;
INSERT INTO test SELECT number from numbers(6);
...
SELECT * FROM (SELECT 1 AS a) PASTE JOIN (SELECT 2 AS b) PASTE JOIN (SELECT 3 AS a) SETTINGS allow_experimental_analyzer = 1;
...
SELECT * FROM test1 `test2.a` PASTE JOIN test2 `test2.a`;
SELECT * FROM test1 PASTE JOIN (SELECT number AS a FROM numbers(2) ORDER BY number DESC);