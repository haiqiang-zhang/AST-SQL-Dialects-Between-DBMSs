...
SELECT * FROM t1 JOIN t2 ON toInt8(1);
SELECT * FROM t1 JOIN t2 ON 256;
SELECT * FROM t1 JOIN t2 ON -1;
SELECT * FROM t1 JOIN t2 ON toString(1);
SELECT '- ON NULL -';
SELECT '- inner -';
...
SELECT * FROM t1 JOIN t2 ON t1.id = t2.id AND 'aaa';
SELECT * FROM t1 JOIN t2 ON 'aaa';
SELECT * FROM t1 JOIN t2 ON t1.id = t2.id AND 0 SETTINGS allow_experimental_analyzer = 0;
