...
grouped = df_expanded.groupby(["SQL_Type", "Result"]).size().unstack(fill_value=0)
...
grouped = grouped.sort_values(by=['SAME_Rate'], ascending=[False])

# # to_csv
grouped.to_csv('classification_rate_total.csv')
